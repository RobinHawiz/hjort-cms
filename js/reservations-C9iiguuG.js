import{i as c,a as p,T as m,b as g,c as h}from"./isResponseError-BQlT-cTs.js";const a=n=>n.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace('"',"&quot;").replace("'","&#039;");function f(){const n=".loader-wrapper",e=document.querySelector(n);c(e,n)&&e.classList.add("remove")}async function v(){try{return(await fetch("https://hjort-backend.azurewebsites.net/api/auth",{headers:{Authorization:"Bearer "+localStorage.getItem("token")}})).ok}catch(n){return console.log("Unexpected app error:",n),!1}}async function d(n,e={}){try{const t=await fetch(n,e);if(!t.ok){const o=await t.json();throw Array.isArray(o)&&o.every(r=>p(r))?o:p(o)?[o]:[{field:"unknown",message:"Unknown error"}]}return t.status===200?await t.json():void 0}catch(t){throw t instanceof TypeError?[{field:"network",message:"Could not connect to service."}]:t instanceof SyntaxError?[{field:"internal",message:"Failed to parse the response body. The response may be empty or the route may not be returning JSON as expected."}]:t}}class u{constructor(e="https://hjort-backend.azurewebsites.net/api/protected"){this.apiUrl=e}async getAll(){return await d(`${this.apiUrl}/reservations`,{method:"GET",headers:{Authorization:"Bearer "+localStorage.getItem("token")}})}async delete(e){const t={method:"DELETE",headers:{Authorization:"Bearer "+localStorage.getItem("token")}};await d(`${this.apiUrl}/reservations/${e}`,t)}}async function w(){try{return await new u().getAll()}catch(n){console.error(n)}}async function y(n){const e=document.createDocumentFragment();return await Promise.all(n.map(async t=>{const o=document.createElement("article");o.dataset.id=t.id;const r=document.createElement("button");r.classList.add("toggle-content"),r.setAttribute("aria-controls","content"),r.innerHTML=`
      ...
      <div class="col-1">
        ...
        <div class="wrapper-container">
          ...
          <div class="booking-date-wrapper wrapper">
            ...
            <p>${i.toLocaleDateString("sv-SE")}</p>
          </div>
          <div class="booking-time-wrapper wrapper">
            <img src="../clock.svg" alt="Klocka ikon" />
            <p>${i.toLocaleTimeString("sv-SE",{hour:"2-digit",minute:"2-digit"})}</p>
          </div>
        </div>
      </div>
      <div class="col-2">
  ...
  `,t.message.length===0&&o.setAttribute("contains-message","true"),o.querySelector(".col-2").appendChild(r),o.querySelector(".email-cancel-button-container").appendChild(s),o.style.maxHeight=await T(o.querySelector(".top-card-info"))+"px",e.appendChild(o)})),e}function E(n,e){const t="#content",o=e.querySelector(t),r=".toggle-content p",l=e.querySelector(r),s=".top-card-info",i=e.querySelector(s);c(o,t)&&c(l,r)&&c(i,s)&&(e.style.maxHeight=e.classList.toggle("expanded")?(i.scrollHeight+o.scrollHeight)/10+"rem":i.scrollHeight/10+"rem",n.toggleOpen(),l.innerText=o.classList.toggle("show")?"Dölj":"Visa")}async function T(n){const e=document.createElement("article");return e.innerHTML=n.outerHTML,e.style.position="absolute",e.style.pointerEvents="none",e.style.visibility="hidden",document.body.appendChild(e),await new Promise(t=>{setTimeout(()=>{t(e.offsetHeight),document.body.removeChild(e)},100)})}async function k(n){if(confirm("Vill du verkligen avboka den här reserveringen?")){const e=new u;try{await e.delete(n.dataset.id),n.remove()}catch(t){console.error(t)}}}function S(n){const e="section",t=document.querySelector(e);c(t,e)&&t.appendChild(n)}async function b(){await v()||window.location.replace("/hjort-cms/"),g(),h();const n=await w();if(!n)return;const e=await y(n);S(e),f()}b();
